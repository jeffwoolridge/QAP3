// NeighborsA component to display countries with neighbors starting with "A"
function NeighborsA({ countries, getCountryNameByCCA3 }) {
  return (
    <div>
      <h2>Countries with neighbors starting with "A"</h2>

      {/* Map through the countries array and filter neighbors starting with "A" */}
      {countries.map((country) => {
        const neighborsWithA = country.borders
          ? country.borders
              .map((code) => getCountryNameByCCA3(code))
              .filter((name) => name && name.startsWith("A"))
          : [];
        
        // Render each country with its neighbors starting with "A"
        return (
          <div
            key={country.cca3}
            style={{
